...
<div class={cn(``, className)}>
    <Dialog.Root>
        <Dialog.Trigger class="bg-black text-sm text-white p-2 md:p-4 flex flex-row items-center gap-4 
        ...
        <Dialog.Overlay class="bg-black/60">
            <Dialog.Content class="p-0 flex flex-col gap-6 ring-0 md:min-w-lg lg:min-w-xl xl:min-w-2xl bg-transparent 
                overflow-y-auto max-h-[90vh]" 
                showCloseButton={false}>
                <Dialog.Header class="flex flex-row justify-end">
                    <Dialog.Close class={cn(buttonVariants({ variant: 'ghost' }), 
                        'text-white tracking-widest hover:text-primary active:text-primary text-lg uppercase cursor-pointer')}>
                        Close
                    </Dialog.Close>
                </Dialog.Header>
                <div>
                    <img src={data.item?.images?.gallery} alt={data.item?.name} class="object-contain w-full h-full" />
                </div>
            </Dialog.Content>
        </Dialog.Overlay>
    </Dialog.Root>
